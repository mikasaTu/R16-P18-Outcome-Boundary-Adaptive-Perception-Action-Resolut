...
def make_env(task: str, num_envs: int = 1):
    ...
    control, horizon, _ = TASKS[task]
    raw = gym.make(task, num_envs=num_envs, sim_backend="physx_cpu", render_backend="sapien_cuda", reconfiguration_freq=1, control_mode=control, reward_mode="normalized_dense", obs_mode="rgb", render_mode="rgb_array", max_episode_steps=horizon)
...
def evaluate_episode(env, agent, seed: int, device: torch.device, visual: str, action_mode: str, tile: int | None = None, horizon: int | None = None, prefix_actions: list | None = None, treatment_steps: int | None = None, continuation_steps: int | None = None, tile_grid: int = 2) -> dict:
    ...
    accounting.update(gpu_latency_ms=0.0, simulator_latency_ms=0.0, action_opportunities=0, executed_steps=0, peak_memory_bytes=0)
    success_trace, reward_trace, cached = [], [], None
    last = None
    if prefix_actions:
    ...
    for step in range(total):
        ...
        current_tile = choose_tile(obs) if tile is None or not in_treatment else tile
    ...
    return {
        ...
        "intended_contact": False, "unintended_contact": False, "collision": False,
        ...
        "success_trace": success_trace, "reward_trace": reward_trace, "accounting": accounting,
    }

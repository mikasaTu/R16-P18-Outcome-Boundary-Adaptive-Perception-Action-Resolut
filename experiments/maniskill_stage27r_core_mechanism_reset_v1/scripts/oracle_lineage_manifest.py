...
def source_record(repo: Path, launcher: Path, *, role: str, pai_run_id: str, pai_job_id: str) -> dict:
    return {
        ...
        "job_id_binding": "runtime wrapper did not inject PAI_CANARY_JOB_ID; external registry completion evidence is required" if pai_job_id in ("", "unknown") else "runtime-bound",
    }

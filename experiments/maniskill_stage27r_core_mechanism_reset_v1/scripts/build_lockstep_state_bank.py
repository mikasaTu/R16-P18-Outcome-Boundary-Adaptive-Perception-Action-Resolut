...
def fidelity(task, seed, prefix):
    env=make_env(task,2)
    try:
      obs,_=env.reset(seed=[seed,seed]); max_action=max_trans=max_rot=0.0; max_rgb=0; categorical=True
      for raw in prefix:
      ...
      return {"broadcast_action_max_abs":max_action,"translation_m":max_trans,"rotation_rad":max_rot,"rgb_max_lsb":max_rgb,"categorical_agreement":categorical,"pass":passed}
    finally: env.close()
...
def expert_rows(h5_path, count_per_phase, offset):
    ...
    with h5py.File(h5_path,"r") as source:
      ...
      if len(keys)<needed: raise RuntimeError(f"expert source has {len(keys)}, need {needed}")
    ...
    for phase_index,phase in enumerate(PHASES):
      for offset in range(count_per_phase):
        ...
        for step in range(branch):
          ...
          action=cached[:,step%4]; prefix.append(action[0].detach().cpu().tolist()); obs,_,_,_,_=env.step(action)
        rows.append({"source_type":"on_policy","source_episode":f"seed_{seed}","phase":phase,"episode_seed":seed,"branch_step":branch,"prefix_actions":prefix})
    return rows
...
def main():
    ...
    pass_rate=float(np.mean([row["fidelity"]["pass"] for row in rows])); atomic_json(a.output,{"protocol_id":PROTOCOL_ID,"task":a.task,"model_seed":a.model_seed,"bank":a.bank,"states":rows,"count":len(rows),"fidelity_pass_rate":pass_rate,"causal_gate_pass":pass_rate>=0.95})

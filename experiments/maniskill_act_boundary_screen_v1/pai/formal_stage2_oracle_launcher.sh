...
STATE_BANK_ROOT="$DATA_ROOT/state_bank"
ORACLE_ROOT="$DATA_ROOT/oracle_atlas"
CHECKPOINT_ROOT=/mnt/cpfs/zbl-cpfs-new/CKPT/leon/torch/r16-p18-maniskill-act-boundary-screen-v1
BASELINE_GATE="$CHECKPOINT_ROOT/baseline_evaluation/baseline_gate.json"
...
on_error() {
  local status=$?
  printf 'R16P18_ORACLE_FAILED line=%s exit=%s command=%q\n' \
...
case "$ARTIFACT_DIR" in
  /mnt/cpfs/zbl-cpfs-new/USERS/leon/logs/r16-p18-maniskill-act-boundary-screen-v1/pai-oracle/*) ;;
  *) printf 'artifact directory escaped fixed oracle log root\n' >&2; exit 71 ;;
esac
case "$STATE_BANK_ROOT" in
  /mnt/cpfs/zbl-cpfs-new/dataset/leon/r16-p18-maniskill-act-boundary-screen-v1/state_bank) ;;
  *) exit 72 ;;
esac
case "$ORACLE_ROOT" in
  /mnt/cpfs/zbl-cpfs-new/dataset/leon/r16-p18-maniskill-act-boundary-screen-v1/oracle_atlas) ;;
  *) exit 73 ;;
esac
case "$CACHE_DIR" in
...
for path in "$ARTIFACT_DIR" "$SELECTED_RAW_ROOT" "$CHECKPOINT_ROOT" \
    "$STATE_BANK_ROOT" "$ORACLE_ROOT" "$CACHE_DIR"; do
  test -d "$path"
  test -w "$path"
  test "$(realpath -e "$path")" = "$path"
  probe="$path/.r16p18-oracle-owner-probe.$$"
  : >"$probe"
...
"$PYTHON" "$EXPERIMENT_ROOT/scripts/run_oracle_matrix.py" \
  --python "$PYTHON" \
  --baseline-gate "$BASELINE_GATE" \
  --selected-raw-root "$SELECTED_RAW_ROOT" \
  --checkpoint-root "$CHECKPOINT_ROOT" \
  --state-bank-root "$STATE_BANK_ROOT" \
  --oracle-root "$ORACLE_ROOT" \
  --artifact-dir "$ARTIFACT_DIR" \
  --gpu-count 2

test -s "$ARTIFACT_DIR/FIRST_REAL_WORK.json"
test -s "$ARTIFACT_DIR/ORACLE_MATRIX_COMPLETE.json"
test -s "$ORACLE_ROOT/oracle_gate.json"
test "$(stat -c '%u:%g' "$ARTIFACT_DIR/ORACLE_MATRIX_COMPLETE.json")" = 2254:2254
sync -f "$ORACLE_ROOT/oracle_gate.json"
printf 'R16P18_FORMAL_STAGE2_ORACLE_COMPLETE run_id=%s\n' "$RUN_ID"

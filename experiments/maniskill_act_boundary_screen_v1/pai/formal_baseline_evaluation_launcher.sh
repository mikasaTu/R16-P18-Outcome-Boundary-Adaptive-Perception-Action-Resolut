...
EVALUATION_ROOT="$CHECKPOINT_ROOT/baseline_evaluation"
SEED_MANIFEST="$EXPERIMENT_ROOT/manifests/data_selection_summary.json"
...
on_error() {
  local status=$?
  printf 'R16P18_BASELINE_EVAL_FAILED line=%s exit=%s command=%q\n' \
...
case "$ARTIFACT_DIR" in
  /mnt/cpfs/zbl-cpfs-new/USERS/leon/logs/r16-p18-maniskill-act-boundary-screen-v1/pai-eval/*) ;;
  *) printf 'artifact directory escaped fixed evaluation log root\n' >&2; exit 71 ;;
esac
case "$EVALUATION_ROOT" in
  /mnt/cpfs/zbl-cpfs-new/CKPT/leon/torch/r16-p18-maniskill-act-boundary-screen-v1/baseline_evaluation) ;;
  *) exit 72 ;;
esac
case "$CACHE_DIR" in
...
for path in "$ARTIFACT_DIR" "$CHECKPOINT_ROOT" "$EVALUATION_ROOT" "$CACHE_DIR"; do
  test -d "$path"
  test -w "$path"
  test "$(realpath -e "$path")" = "$path"
  probe="$path/.r16p18-eval-owner-probe.$$"
  : >"$probe"
...
"$PYTHON" "$EXPERIMENT_ROOT/scripts/run_closed_loop_matrix.py" \
  --python "$PYTHON" \
...
sync -f "$EVALUATION_ROOT/baseline_gate.json"
printf 'R16P18_BASELINE_EVALUATION_COMPLETE run_id=%s\n' "$RUN_ID"
